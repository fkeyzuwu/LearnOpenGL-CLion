...
int main()
{
    ...
    unsigned int texture1 = generateTexture("../wood.jpg", GL_RGB);
    unsigned int texture2 = generateTexture("../awesomeface.png", GL_RGBA);
...
void processInput(GLFWwindow *window)
{
    ...
    else if(glfwGetKey(window, GLFW_KEY_UP) == GLFW_PRESS && mixAmount < 1.0f)
    {
        mixAmount += 0.0001f;
    }
    else if(glfwGetKey(window, GLFW_KEY_DOWN) == GLFW_PRESS && mixAmount > 0.0f)
    {
        mixAmount -= 0.0001f;
    }
}
